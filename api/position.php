...
function getList($pdo) {
    ...
    if (isset($_GET['is_recommend'])) {
        $isRecommend = (int)$_GET['is_recommend'];
        $whereConditions[] = "is_recommend = :is_recommend";
    ...
    // 查询总数
    $stmt = $pdo->prepare("SELECT COUNT(*) as total FROM positions {$whereClause}");
    $stmt->execute($params);
    ...
    $stmt = $pdo->prepare("SELECT * FROM positions {$whereClause} ORDER BY {$orderBy} {$order} LIMIT {$offset}, {$pageSize}");
    $stmt->execute($params);
 ...
 * 根据国家获取岗位分类列表
 */
function getCategoriesByCountry($pdo) {
    $country = $_GET['country'] ?? '';
    
    if (empty($country) || $country === '全部国家') {
        // 没有选择国家或选择"全部国家"时，返回所有分类
        $stmt = $pdo->query("SELECT DISTINCT category FROM positions WHERE category IS NOT NULL AND category != '' ORDER BY category");
    } else {
        // 根据指定国家获取分类
        $stmt = $pdo->prepare("SELECT DISTINCT category FROM positions WHERE country = :country AND category IS NOT NULL AND category != '' ORDER BY category");
