...
function loadCustomerProgress(customerId) {
    fetch(`../api/customer.php?action=progress&id=${customerId}`)
                            ...
                            if (Array.isArray(materials) && materials.length > 0) {
                                const badges = materials.map(m => {
                                    ...
                                    if (status === 'uploaded' || status === 'approved') {
                                        return `<span class="material-badge uploaded" title="已上传">${m}</span>`;
                                    } else {
        ...
        .catch(err => console.error('加载进度失败:', err));
}
...
function getSelectedMaterials() {
    const selected = [];
    const allTags = document.querySelectorAll('#materialsTags .material-tag');
    console.log('所有标签数量:', allTags.length);
    
    allTags.forEach(btn => {
        console.log('标签:', btn.dataset.value, '选中状态:', btn.classList.contains('selected'));
    ...
    .then(res => {
        ...
        if (res.code === 200) {
            ...
            if (selectedMaterials.length > 0) {
                const badges = selectedMaterials.map(m => `<span class="material-badge pending" title="未上传">${m}</span>`).join('');
                materialsHtml = `
                    <div class="progress-materials">
                        <span class="materials-label">📋</span>
                        ${badges}
            ...
            console.log('生成的 HTML:', materialsHtml);
