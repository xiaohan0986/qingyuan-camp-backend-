...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        if ($_POST['action'] === 'list') {
            ...
            if ($keyword !== '') {
                $where[] = '(p.name LIKE ? OR p.phone LIKE ? OR w.bank_account LIKE ? OR w.account_name LIKE ?)';
                $params[] = "%{$keyword}%";
                $params[] = "%{$keyword}%";
                $params[] = "%{$keyword}%";
                $params[] = "%{$keyword}%";
            ...
            $countRow = $db->fetchOne("SELECT COUNT(*) AS total FROM app_withdraws w LEFT JOIN app_partners p ON w.partner_id = p.id WHERE {$whereSql}", $params);
            $total = intval($countRow['total'] ?? 0);

            $rows = $db->fetchAll("
                SELECT w.*, p.name AS partner_name, p.phone AS partner_phone, p.level AS partner_level, p.balance AS partner_balance
        ...
        if ($_POST['action'] === 'audit') {
            ...
            $row = $db->fetchOne("SELECT id, status, amount, partner_id FROM app_withdraws WHERE id = ?", [$id]);
            if (!$row) {
                echo json_encode(['success' => false, 'message' => '记录不存在']);
                exit;
            }
            ...
            // 开启事务（使用 mysqli 原生连接避免 prepared statement 限制）
            $conn = $db->getConnection();
            $conn->begin_transaction();
            try {
                $db->update('app_withdraws', [
                    'status' => $status,
                    'remark' => $remark,
                ...
                if ($status == 1) {
                    // 通过：扣减余额，累加已提现，记录收益
                    $stmt = $conn->prepare("UPDATE app_partners SET balance = balance - ?, withdrawn = withdrawn + ? WHERE id = ?");
                    $amt = floatval($row['amount']);
                    $pid = intval($row['partner_id']);
                    $stmt->bind_param('ddi', $amt, $amt, $pid);
                    $stmt->execute();
                    $stmt->close();

                    $db->insert('app_earnings', [
                        'partner_id' => $pid,
                        'type' => 'withdraw',
                        'amount' => -$amt,
                        'source' => '提现申请',
                        'description' => '提现审核通过：¥' . number_format($amt, 2),
        ...
        if ($_POST['action'] === 'batch_audit') {
            ...
            try {
                ...
                $db->query("UPDATE app_withdraws SET status = ?, handled_at = NOW() WHERE status = 0 AND id IN ({$placeholders})", $params);

                if ($status == 1) {
                    foreach ($ids as $wid) {
                        $w = $db->fetchOne("SELECT partner_id, amount FROM app_withdraws WHERE id = ?", [$wid]);
                        if ($w) {
                            $stmt = $conn->prepare("UPDATE app_partners SET balance = balance - ?, withdrawn = withdrawn + ? WHERE id = ?");
                            $amt = floatval($w['amount']);
                            $pid = intval($w['partner_id']);
                            $stmt->bind_param('ddi', $amt, $amt, $pid);
                            $stmt->execute();
                            $stmt->close();
        ...
        if ($_POST['action'] === 'delete') {
            $id = intval($_POST['id'] ?? 0);
            $row = $db->fetchOne("SELECT id, status FROM app_withdraws WHERE id = ?", [$id]);
            if (!$row) {
                echo json_encode(['success' => false, 'message' => '记录不存在']);
                exit;
            }
            if ($row['status'] == 1) {
                echo json_encode(['success' => false, 'message' => '已通过的申请不能直接删除，请先通过 app_partners 调账']);
                exit;
            }
            $db->delete('app_withdraws', 'id = ?', [$id]);
        ...
        if ($_POST['action'] === 'batch_delete') {
            ...
            $ids = array_map('intval', $ids);
            $placeholders = implode(',', array_fill(0, count($ids), '?'));
            $db->query("DELETE FROM app_withdraws WHERE status <> 1 AND id IN ({$placeholders})", $ids);
            echo json_encode(['success' => true, 'message' => '批量删除成功（已通过的记录已跳过）']);
            exit;
        }

        if ($_POST['action'] === 'stats') {
            $total = $db->fetchOne("SELECT COALESCE(SUM(amount), 0) AS a, COUNT(*) AS c FROM app_withdraws")['a'] ?? 0;
            $totalCount = $db->fetchOne("SELECT COUNT(*) AS c FROM app_withdraws")['c'] ?? 0;
            $pending = $db->fetchOne("SELECT COALESCE(SUM(amount), 0) AS a, COUNT(*) AS c FROM app_withdraws WHERE status = 0")['a'] ?? 0;
            $pendingCount = $db->fetchOne("SELECT COUNT(*) AS c FROM app_withdraws WHERE status = 0")['c'] ?? 0;
            $approved = $db->fetchOne("SELECT COALESCE(SUM(amount), 0) AS a, COUNT(*) AS c FROM app_withdraws WHERE status = 1")['a'] ?? 0;
            $rejected = $db->fetchOne("SELECT COALESCE(SUM(amount), 0) AS a, COUNT(*) AS c FROM app_withdraws WHERE status = 2")['a'] ?? 0;
            echo json_encode([
                'success' => true,
                ...
                'approved' => floatval($approved),
                'rejected' => floatval($rejected),
            ]);
            exit;
        }
...
$stats = [
    'total' => floatval($db->fetchOne("SELECT COALESCE(SUM(amount), 0) AS a FROM app_withdraws")['a'] ?? 0),
    'total_count' => intval($db->fetchOne("SELECT COUNT(*) AS c FROM app_withdraws")['c'] ?? 0),
    'pending' => floatval($db->fetchOne("SELECT COALESCE(SUM(amount), 0) AS a FROM app_withdraws WHERE status = 0")['a'] ?? 0),
    'pending_count' => intval($db->fetchOne("SELECT COUNT(*) AS c FROM app_withdraws WHERE status = 0")['c'] ?? 0),
    'approved' => floatval($db->fetchOne("SELECT COALESCE(SUM(amount), 0) AS a FROM app_withdraws WHERE status = 1")['a'] ?? 0),
    'rejected' => floatval($db->fetchOne("SELECT COALESCE(SUM(amount), 0) AS a FROM app_withdraws WHERE status = 2")['a'] ?? 0),
];
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <style>
        ...
        .cell-amount { color: #fa8c16; font-weight: 700; font-size: 15px; }
...
<body class="admin-body">
    ...
    <div class="main-content">
        ...
        <div class="content-wrapper">
            ...
            <div class="stats-row">
                <div class="stat-card">
                    <div class="label">提现总金额</div>
                    <div class="value">¥<?= number_format($stats['total'], 2) ?></div>
                    <div class="sub">共 <?= $stats['total_count'] ?> 笔</div>
                </div>
                <div class="stat-card orange">
                    <div class="label">待审核金额</div>
                    <div class="value" style="color:#fa8c16;">¥<?= number_format($stats['pending'], 2) ?></div>
                    <div class="sub">共 <?= $stats['pending_count'] ?> 笔待处理</div>
                </div>
                <div class="stat-card green">
                    <div class="label">已通过金额</div>
                    <div class="value" style="color:#52c41a;">¥<?= number_format($stats['approved'], 2) ?></div>
                </div>
                <div class="stat-card red">
                    <div class="label">已拒绝金额</div>
                    <div class="value" style="color:#ff4d4f;">¥<?= number_format($stats['rejected'], 2) ?></div>
                </div>
            </div>

            <div class="search-bar">
                <input type="text" id="searchKeyword" placeholder="🔍 合伙人姓名/电话/银行卡/账户名">
    ...
    <div class="modal" id="auditModal">
        <div class="modal-content">
            <div class="modal-header" id="auditTitle">审核提现申请</div>
            <div class="modal-body">
                ...
                <form id="auditForm">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="remark" id="auditRemark" placeholder="可填写审核意见/打款流水号等..."></textarea>
                    </div>
                </form>
            </div>
    ...
    <div class="modal" id="detailModal">
        <div class="modal-content">
            <div class="modal-header">💰 提现申请详情</div>
    ...
    function renderTable(rows) {
        ...
        if (!rows || rows.length === 0) {
            container.innerHTML = `<div class="empty-state"><div class="icon">💰</div><div class="text">暂无提现申请记录</div></div>`;
            return;
        }
        let html = `
            <table>
        ...
        rows.forEach(r => {
            ...
            const bank = `${escapeHtml(r.bank_name || '-')} <span style="color:#8c8c8c;font-size:12px;">${escapeHtml(r.bank_account || '')}</span> · ${escapeHtml(r.account_name || '-')}`;
            html += `
                <tr data-id="${r.id}">
                    <td><input type="checkbox" class="checkbox item-checkbox" value="${r.id}"></td>
                    <td>#${r.id}</td>
                    <td>
                        <div style="color:var(--text-primary);font-weight:500;">${partner}</div>
                        <div class="cell-meta">${escapeHtml(r.partner_level || '')} · ${escapeHtml(r.partner_phone || '')}</div>
                    </td>
                    <td><span class="cell-amount">¥${parseFloat(r.amount || 0).toFixed(2)}</span></td>
                    <td>${bank}</td>
                    <td>${statusBadge}</td>
                    <td>${created}</td>
                    <td>${handled}</td>
    ...
    function openAuditModal(id, status) {
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('auditTitle').textContent = status == 1 ? '✅ 通过提现' : '❌ 拒绝提现';
                document.getElementById('auditQuestion').textContent = status == 1 ? '通过审核（将自动扣减合伙人余额）' : '拒绝审核（余额不变）';
                document.getElementById('auditInfo').innerHTML = `
                    <div class="info-item"><div class="label">合伙人</div><div class="value">${escapeHtml(r.partner_name || '-')}（${escapeHtml(r.partner_level || '')}）</div></div>
                    <div class="info-item"><div class="label">联系电话</div><div class="value">${escapeHtml(r.partner_phone || '-')}</div></div>
                    <div class="info-item"><div class="label">提现金额</div><div class="value cell-amount">¥${parseFloat(r.amount || 0).toFixed(2)}</div></div>
                    <div class="info-item"><div class="label">当前余额</div><div class="value" style="color:${r.partner_balance < r.amount ? '#ff4d4f' : '#52c41a'};">¥${parseFloat(r.partner_balance || 0).toFixed(2)}</div></div>
                    <div class="info-item" style="grid-column:span 2;"><div class="label">收款账户</div><div class="value">${escapeHtml(r.bank_name || '-')} ${escapeHtml(r.bank_account || '')} (${escapeHtml(r.account_name || '')})</div></div>
                `;
    ...
    function submitAudit() {
        ...
        // 余额检查
        if (currentDetail && status == 1 && parseFloat(currentDetail.partner_balance || 0) < parseFloat(currentDetail.amount || 0)) {
            if (!confirm('⚠️ 该合伙人当前余额（¥' + parseFloat(currentDetail.partner_balance).toFixed(2) + '）少于提现金额（¥' + parseFloat(currentDetail.amount).toFixed(2) + '），仍要通过吗？')) {
                return;
            }
        }
    ...
    function batchAudit(status) {
        ...
        if (!confirm(`确定要${action}选中的 ${ids.length} 条提现申请吗？` + (status == 1 ? '\n（通过将自动扣减合伙人余额）' : ''))) return;
        const params = new URLSearchParams();
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('detailBody').innerHTML = `
                    <div class="form-group"><label>提现金额</label><div class="cell-amount" style="font-size:24px;">¥${parseFloat(r.amount || 0).toFixed(2)}</div></div>
                    <div class="form-group"><label>状态</label><div>${statusHtml}</div></div>
                    <div class="form-group"><label>合伙人</label><div>${escapeHtml(r.partner_name || '-')} (${escapeHtml(r.partner_level || '')})</div></div>
                    <div class="form-group"><label>联系电话</label><div>${escapeHtml(r.partner_phone || '-')}</div></div>
                    <div class="form-group"><label>收款银行</label><div>${escapeHtml(r.bank_name || '-')}</div></div>
                    <div class="form-group"><label>银行卡号</label><div style="font-family:monospace;">${escapeHtml(r.bank_account || '-')}</div></div>
                    <div class="form-group"><label>账户姓名</label><div>${escapeHtml(r.account_name || '-')}</div></div>
